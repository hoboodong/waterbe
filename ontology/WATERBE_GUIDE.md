...
```
─── STEP 1. 기본 정보 확인 ───────────────────────────────
  아래 정보를 사용자에게 확인한다 (모르는 것만 물어볼 것):
  - 상품명
  - 매장
  - 소비자가 (products.yaml에 있으면 자동 참조)
  - 신규인지 기존 변경인지
...
─── STEP 3. 재료 구성 협의 ───────────────────────────────
  [기존 레시피 변경 시]
    현재 레시피 원가를 예시 2 방식으로 계산해서 먼저 보여준다
    현재 원가 vs 한도 비교 → 초과/여유 금액 표시

  [신규 or 재구성 시]
    사용자가 넣고 싶은 재료를 말하면
    → 각 재료의 원가/g 즉시 계산해서 보여주며 협의
  ...
  │ ...                                      │
  │ ─────────────────────────────────────── │
  │ 현재 재료비  x,xxx원 / 예산 x,xxx원     │
  │ 여유       +xxx원  ← 추가 재료 가능     │
  └──────────────────────────────────────────┘

─── STEP 4. 확정 및 저장 ─────────────────────────────────
  최종 레시피 요약 후 사용자 확인
  확인되면 recipes/{매장}.yaml에 저장
  신규 상품이면 products.yaml에도 추가 여부 확인

  저장 시 주의:
  - 기존 레시피 변경 → 기존 레코드 수정 (이력 관리 불필요)
  - 매장 추가 → 해당 매장 recipe 파일에 신규 레코드 추가
```
